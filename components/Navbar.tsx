...
const Navbar: React.FC<NavbarProps> = ({ onOpenChat, setView, currentView }) => {
  ...
  const navigate = (view: string) => {
    if (setView) {
      ...
      // Fallback for direct browser navigation if router isn't passed
      window.location.href = view === 'home' ? '/' : `/${view}`;
    }
  };

  return (
    <nav className={`fixed top-0 left-0 right-0 z-[100] px-6 md:px-12 py-8 transition-all duration-500 ${!isDark ? 'bg-white/80 backdrop-blur-md py-4 shadow-sm border-b border-gray-100' : ''}`}>
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <div className="flex items-center gap-10">
          <div className={`hidden lg:flex items-center gap-10 text-[10px] font-black uppercase tracking-[0.2em] transition-colors ${!isDark ? 'text-gray-400' : 'text-white/40'}`}>
            <button onClick={() => navigate('home')} className={`hover:text-blue-600 transition-colors ${currentView === 'home' ? 'text-blue-600' : ''}`}>Home</button>
            <button onClick={() => navigate('work-list')} className={`hover:text-blue-600 transition-colors ${currentView === 'work-list' || currentView === 'work' ? 'text-blue-600' : ''}`}>Work</button>
            <button onClick={() => navigate('resume')} className={`hover:text-blue-600 transition-colors ${currentView === 'resume' ? 'text-blue-600' : ''}`}>CV</button>
            <button onClick={() => navigate('contact')} className={`hover:text-blue-600 transition-colors ${currentView === 'contact' ? 'text-blue-600' : ''}`}>Contact</button>
          </div>
          <button 
            onClick={onOpenChat}
